...
void greedy_search(const Input& in, Output& out, Map& mp) {
    ...
    while(count < 2*in.N*in.N && !mp.all_filled) {
        ...
        if (best_new_cells <= 0) {
            ...
            // 見つかった経路に沿って移動（最長の経路の長さまで）
            int max_path_length = 0;
            rep(j, 0, in.M) if (has_path[j]) max_path_length = max(max_path_length, (int)paths[j].size());
            
            for (int step = 0; step < max_path_length; step++) {
                // 各ステップで最適なパターンを選択
                int best_pattern_step = 0;
                double best_match_score = -1.0;
                
                rep(pattern, 0, in.K) {
                    double match_score = 0.0;
                    rep(j, 0, in.M) {
                        if (has_path[j] && step < (int)paths[j].size()) {
                            if (out.c[pattern][j] == paths[j][step]) {
                                match_score += 1.0;
                            }
                        }
                    }
...
int main(){
    ...
    rep(i,0,N) {
        string S;cin >> S;
        rep(j,0,N-1) {
            S[j] == '1' ? in.v[i][j] = 1 : in.v[i][j] = 0;
        }
    }
    rep(i,0,N-1) {
        string S;cin >> S;
        rep(j,0,N) {
            S[j] == '1' ? in.h[i][j] = 1 : in.h[i][j] = 0;
        }
    }
    ...
    rep(i, 4, K) rep(j, 0, M) out.c[i][j] = put[rand()%4];
    Map mp(N, in.p);
