    ...
    auto specialCellsInterfere = [&](const pl& c1, const pl& c2) -> bool {
        ...
        rep(p, 0, numPaths) {
            ...
            for (const auto& cell : route.paths[p]) {
                if (cell == c1) has1 = true;
                if (cell == c2) has2 = true;
            }
            if (has1 && has2) return true;
        }
        
        // 異なる経路だが状態が同じ（色が同じ）場合も干渉の可能性
        // より厳密には、実行順序を考慮する必要があるが、保守的に判定
        set<ll> paths1, paths2;
        rep(p, 0, numPaths) {
            for (const auto& cell : route.paths[p]) {
                if (cell == c1) paths1.insert(pathColor[p]);
                if (cell == c2) paths2.insert(pathColor[p]);
            }
        }
        
        // 共通の状態色を持つ場合は干渉
        for (auto pc1 : paths1) {
            if (paths2.count(pc1)) return true;
        }
...
// MakeRuleAndBoardByPath関数内の最後に追加

    cerr << "Generated rules: C=" << rule.C << ", Q=" << rule.Q << ", M=" << rule.M << endl;
    
    // 色の詰め直し（実際に使用されている色だけに番号を振り直す）
    set<ll> usedColors;
    
    // 盤面で使用されている色を収集
    rep(i, 0, N) {
        rep(j, 0, N) {
            usedColors.insert(board.colors[i][j]);
        }
    }
    
    // ルールで使用されている色を収集
    for (auto& [state, a] : rule.A) {
        usedColors.insert(state.first);  // 入力色
        usedColors.insert(a);            // 出力色
    }
    
    cerr << "Used colors before compaction: " << usedColors.size() << " colors" << endl;
    
    // 色の再マッピング（0から連番に）
    map<ll, ll> colorRemap;
    ll newColorId = 0;
    for (ll oldColor : usedColors) {
        colorRemap[oldColor] = newColorId++;
    }
    
    cerr << "Color compaction: " << rule.C << " -> " << newColorId << " colors" << endl;
    
    // 盤面の色を更新
    rep(i, 0, N) {
        rep(j, 0, N) {
            board.colors[i][j] = colorRemap[board.colors[i][j]];
        }
    }
    
    // ルールの色を更新
    Rule newRule;
    for (auto& state : rule.states) {
        ll oldInputColor = state.first;
        ll oldInternalState = state.second;
        ll oldOutputColor = rule.A[state];
        ll newInternalState = rule.S[state];
        char direction = rule.D[state];
        
        ll newInputColor = colorRemap[oldInputColor];
        ll newOutputColor = colorRemap[oldOutputColor];
        
        newRule.addState(newInputColor, oldInternalState, newOutputColor, newInternalState, direction);
    }
    
    rule = newRule;
    
    cerr << "After compaction: C=" << rule.C << ", Q=" << rule.Q << ", M=" << rule.M << endl;
}
