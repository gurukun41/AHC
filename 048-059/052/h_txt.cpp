...
// 焼きなまし法のために追加する関数と変数
Output anneal(const Input& in, Output& initial_out, Map& mp) {
    // 初期解と初期スコア
    ...
    while (true) {
        ...
        // 指示の一部を変更
        int change_pos = std::uniform_int_distribution<int>(0, current_out.a.size() - 1)(gen);
        int old_pattern = new_out.a[change_pos];
        int new_pattern;
        do {
            new_pattern = std::uniform_int_distribution<int>(0, in.K - 1)(gen);
        } while (new_pattern == old_pattern);
        new_out.a[change_pos] = new_pattern;
        ...
        // 新しい経路でシミュレーション
        new_map.grid = vector<vector<bool>>(in.N, vector<bool>(in.N, false));
        for (auto& p : robots) new_map.grid[p.x][p.y] = true;

        for (int step = 0; step < new_out.a.size(); step++) {
            int pattern = new_out.a[step];

            rep(j, 0, in.M) {
                char move = new_out.c[pattern][j];
                int k = find(dir_c.begin(), dir_c.end(), move) - dir_c.begin();
                ll nx = robots[j].x + dir[k].first;
                ll ny = robots[j].y + dir[k].second;
                ...
                robots[j].x = nx;
                robots[j].y = ny;
                new_map.grid[nx][ny] = true;
            }
        }

        new_map.search_filled();
...
int main() {
    ...
    rep(i, 8, K) rep(j, 0, M) out.c[i][j] = put[rand() % 4];
    ...
    // 焼きなまし法で解を改善
    Output annealed_out = anneal(in, out, mp);
