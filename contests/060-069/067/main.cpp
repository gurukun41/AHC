...
static constexpr uint64_t DEFAULT_RNG_SEED = 88172645463325252ULL;
...
struct XorShift {
    uint64_t x;
    explicit XorShift(uint64_t seed = DEFAULT_RNG_SEED) : x(seed) {}
    uint64_t next() {
        x ^= x << 7;
        x ^= x >> 9;
        return x;
...
class Solver {
    static constexpr double INITIAL_SEARCH_LIMIT = 0.68;
    static constexpr double TREE_SEARCH_LIMIT = 1.58;
    ...
    Gadget find_gadget(const TreeInfo& info) const {
        ...
        for (int main_index = 1; main_index + 1 < (int)main_path.size(); ++main_index) {
            ...
            for (auto [root, root_edge] : tree[hub]) {
                ...
                vector<int> down_depth(V, 0), down_end(V);
                vector<int> best_child(V, -1);
                vector<int> best_depth(V, 0), best_end(V);
                vector<int> second_depth(V, 0), second_end(V);
                iota(down_end.begin(), down_end.end(), 0);
                iota(best_end.begin(), best_end.end(), 0);
                iota(second_end.begin(), second_end.end(), 0);
                for (int oi = (int)order.size() - 1; oi >= 0; --oi) {
                    int v = order[oi];
                    for (int child : children[v]) {
                        int candidate_depth = 1 + down_depth[child];
                        int candidate_end = down_end[child];
                        if (candidate_depth > best_depth[v]) {
                            second_depth[v] = best_depth[v];
                            second_end[v] = best_end[v];
                            best_depth[v] = candidate_depth;
                            best_end[v] = candidate_end;
                            best_child[v] = child;
                        } else if (candidate_depth > second_depth[v]) {
                            second_depth[v] = candidate_depth;
                            second_end[v] = candidate_end;
                        }
                    }
                    down_depth[v] = best_depth[v];
                    down_end[v] = best_end[v];
                }
                ...
                for (int leaf : leaves) {
                    ...
                    for (int index = 0; index < length; ++index) {
                        ...
                        if (best_child[v] != next) {
                            side_depth[index] = best_depth[v];
                            side_end[index] = best_depth[v] > 0 ? best_end[v] : -1;
                        } else {
                            side_depth[index] = second_depth[v];
                            side_end[index] = second_depth[v] > 0 ? second_end[v] : -1;
                        }
                    }
                    ...
                    for (int bits = max_bits; bits >= 1; --bits) {
                        ...
                        for (int count = 2; count <= selected_count; ++count) {
                            ...
                            long long best_value = NEG;
                            int best_index = -1;
                            for (int index = 0; index + 1 < length; ++index) {
                                int prev = index - 1;
                                if (prev >= 0 && dp[count - 1][prev] != NEG) {
                                    long long value =
                                        dp[count - 1][prev] - weight * prev;
                                    if (value > best_value) {
                                        best_value = value;
                                        best_index = prev;
                                    }
                                ...
                                if (best_index != -1) {
                                    dp[count][index] =
                                        best_value
                                        + weight * (index + side_depth[index]);
                                    previous[count][index] = best_index;
                                }
                            }
                        }
    ...
    Candidate make_candidate(vector<char> in_tree) const {
        Candidate candidate;
    ...
    int exact_shortest_path(const vector<Door>& doors, const vector<Switch>& switches) const {
        ...
        if ((int)bfs_seen.size() != STATES) {
            bfs_seen.assign(STATES, 0);
            bfs_dist.resize(STATES);
            bfs_queue.resize(STATES);
            bfs_stamp = 0;
        }
        ...
        while (head < tail) {
            ...
            for (auto [to, eid] : graph[v]) {
                ...
                if (bfs_seen[index] == bfs_stamp) continue;
                bfs_seen[index] = bfs_stamp;
                bfs_dist[index] = current + 1;
                bfs_queue[tail++] = index;
            }
            if (switch_type[v] != -1) {
                int next_mask = mask ^ (1 << switch_type[v]);
                int index = v * MASKS + next_mask;
                if (bfs_seen[index] != bfs_stamp) {
                    bfs_seen[index] = bfs_stamp;
                    bfs_dist[index] = current + 1;
                    bfs_queue[tail++] = index;
                }
...
public:
    void solve() {
        ...
        while (timer.elapsed() < INITIAL_SEARCH_LIMIT) {
            ...
            Candidate candidate = make_candidate(std::move(tree));
            insert_archive(accepted_archive, candidate, 12, better_direct);
        }
        ...
        while (!accepted_archive.empty() && timer.elapsed() < TREE_SEARCH_LIMIT) {
            ...
            Candidate next = make_candidate(std::move(next_tree));
            ...
            if (accept) {
                ...
                insert_archive(accepted_archive, current, 12, better_direct);
                stale = improve ? 0 : stale + 1;
            } else {
                insert_archive(reject_archive, next, 4, better_direct);
                ++stale;
            }
            if (stale >= 60) {
        ...
        auto append_archive = [&](const vector<Candidate>& archive) {
            for (const auto& candidate : archive) {
                uint64_t hash = tree_hash(candidate.in_tree);
                if (exact_index.emplace(hash, (int)promising.size()).second) {
                    promising.push_back(candidate);
                }
            }
        };
        append_archive(reject_archive);
        append_archive(accepted_archive);
