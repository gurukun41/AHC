...
vector<pair<ll, ll>> findInitialBlockingPositions(Map& map) {
    ...
    rep(i, 0, N) {
        ...
        // 処理範囲を決定（iとjが入れ替わっていることに注意）
        rep(j, 0, N) {
            if (start == -1 && map.mapInfo[i][j] == 1 && i > 0 && map.mapInfo[i - 1][j] == 1 && i + 1 < N &&
                map.mapInfo[i + 1][j] == 1) {
                start = j + 1;
            }
            if (end == -1 && map.mapInfo[i][N - 1 - j] == 1 && i > 0 && map.mapInfo[i - 1][N - 1 - j] == 1 &&
                i + 1 < N && map.mapInfo[i + 1][N - 1 - j] == 1) {
                end = N - 2 - j;
            }
        }
        if (start == -1 || end == -1 || start > end) {
            continue;  // 適切な範囲がない場合はスキップ
        }
        ...
        rep(j, 1, N - 1) {
            ...
            if (map.mapInfo[i][j] == 1 && pair<ll, ll>(i, j) != target) {
                // 横のブロックを避ける（iとjが入れ替わっている）
                if (i > 1 && map.mapInfo[i - 1][j + 1] == 0 && map.mapInfo[i - 2][j + 1] == 0) {
                    continue;
                }
                if (i > 1 && map.mapInfo[i - 1][j] == 0 && map.mapInfo[i - 2][j] == 0) {
                    continue;
                }
                if (i > 1 && map.mapInfo[i - 1][j - 1] == 0 && map.mapInfo[i - 2][j - 1] == 0) {
                    continue;
                ...
                // 斜めのブロックを避ける（iとjが入れ替わっている）
                if (i > 1 && map.mapInfo[i - 1][j - 1] == 0 && map.mapInfo[i - 2][j] == 0) {
                    continue;
                }
                if (i > 1 && map.mapInfo[i - 1][j] == 0 && map.mapInfo[i - 2][j + 1] == 0) {
                    continue;
                }
                if (i > 1 && j + 2 < N && map.mapInfo[i - 1][j + 1] == 0 && map.mapInfo[i - 2][j + 2] == 0) {
                    continue;
                }
                if (i > 1 && map.mapInfo[i - 1][j + 1] == 0 && map.mapInfo[i - 2][j] == 0) {
                    continue;
                ...
                if (i > 1 && j - 2 >= 0 && map.mapInfo[i - 1][j - 1] == 0 && map.mapInfo[i - 2][j - 2] == 0) {
                    continue;
                }
...
vector<pair<ll, ll>> findOptimalBlockingPositions(Map& map, bool first) {
    ...
    // 最初のターンなら初期パターンで配置
    if (first) {
        result = findInitialBlockingPositions(map);
