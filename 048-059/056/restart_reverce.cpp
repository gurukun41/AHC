...
struct Route {
    ...
    void optimizeGlobalRoute(double tl) {
        ...
        auto calcMetrics = [&]() -> pair<ll, ll> {
            ...
            return {totalDist, (ll)uniqueCells.size()};
        };
        
        auto [currentDist, currentUnique] = calcMetrics();
        // 評価関数はユニークセル数のみ
        ll best_score = currentUnique;          // 小さいほど良い
        ...
        while (get_time() - start < tl) {
            ...
            // --- usedCells を意識したコスト付き最短路 ---
            auto dijkstra_path = [&](pl startP, pl goalP) -> vpl {
                // 距離（コスト）
                map<pl, double> dist;
                map<pl, pl> parent;
                
                // 優先度付きキュー（距離が小さい順）
                using Node = pair<double, pl>;
                ...
                while (!pq.empty()) {
                    ...
                    for (auto [di, dj] : dirs) {
                        ...
                        // コスト: 基本 1、usedCells が多いほど安く
                        ll use = 0;
                        auto it = usedCells.find(nxt);
                        if (it != usedCells.end()) use = it->second;
                        // α を調整パラメータとする（強く寄せたいなら大きく）
                        const double alpha = 0.3;
                        double edgeCost = max(0.05, 1.0 - alpha * (double)use);
                        
                        double nd = cd + edgeCost;
                        if (!dist.count(nxt) || nd + 1e-9 < dist[nxt]) {
                            dist[nxt] = nd;
                            parent[nxt] = cur;
                            pq.push({nd, nxt});
                        }
                ...
                if (!parent.count(goalP)) return {};
                vpl path;
                pl cur = goalP;
                while (cur != pl{-1, -1}) {
                    path.push_back(cur);
                    if (!parent.count(cur)) break;
                    cur = parent[cur];
                }
                reverse(all(path));
                return path;
            ...
            vpl path_sm = dijkstra_path(s, m);
            vpl path_me = dijkstra_path(m, e);
            ...
            ll new_score = newUnique;   // ユニークセル数のみ
            
            // 焼きなまし判定
            if (new_score < best_score || 
                (temp > 1e-9 && rnd.next_double() < exp((double)(best_score - new_score) / temp))) {
        ...
        cerr << "Best unique cells: " << best_score << endl;
    }
};
...
void solve(int seed = -1) {
    ...
    // 1. 経路の焼きなまし（0.4秒）
    double time_limit = get_time() + 0.4;
