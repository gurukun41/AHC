...
vector<pair<ll, ll>> findInitialBlockingPositions(Map& map) {
    ...
    rep(j, 0, N) {
        ...
        // 処理範囲を決定
        rep(i, 0, N) {
            if (start == -1 && map.mapInfo[i][j] == 1 && j > 0 && map.mapInfo[i][j - 1] == 1 && j + 1 < N &&
                map.mapInfo[i][j + 1] == 1) {
                start = i + 1;
            }
            if (end == -1 && map.mapInfo[N - 1 - i][j] == 1 && j > 0 && map.mapInfo[N - 1 - i][j - 1] == 1 &&
                j + 1 < N && map.mapInfo[N - 1 - i][j + 1] == 1) {
                end = N - 2 - i;
            }
        }
        ...
        rep(i, 1, N - 1) {
            ...
            if (map.mapInfo[i][j] == 1 && pair<ll, ll>(i, j) != target) {
                // 横のブロックを避ける
                if (j > 1 && map.mapInfo[i + 1][j - 1] == 0 && map.mapInfo[i + 1][j - 2] == 0) {
                    continue;
                }
                if (j > 1 && map.mapInfo[i][j - 1] == 0 && map.mapInfo[i][j - 2] == 0) {
                    continue;
                }
                if (j > 1 && map.mapInfo[i - 1][j - 1] == 0 && map.mapInfo[i - 1][j - 2] == 0) {
                    continue;
                ...
                // 斜めのブロックを避ける
                if (j > 1 && map.mapInfo[i - 1][j - 1] == 0 && map.mapInfo[i][j - 2] == 0) {
                    continue;
                }
                if (j > 1 && map.mapInfo[i][j - 1] == 0 && map.mapInfo[i + 1][j - 2] == 0) {
                    continue;
                }
                if (j > 1 && i + 2 < N && map.mapInfo[i + 1][j - 1] == 0 && map.mapInfo[i + 2][j - 2] == 0) {
                    continue;
                }
                if (j > 1 && map.mapInfo[i + 1][j - 1] == 0 && map.mapInfo[i][j - 2] == 0) {
                    continue;
                ...
                if (j > 1 && i - 2 >= 0 && map.mapInfo[i - 1][j - 1] == 0 && map.mapInfo[i - 2][j - 2] == 0) {
                    continue;
                }
