    ...
    rep(j, 0, N) {
        rep(i, 0, N) {
            if (now == 0) {
                ...
                rep(k, 1, 3) {
                    if (j + k >= N) continue;
                    if (map.mapInfo[i][j + k] == 1 && pair<ll, ll>(i, j + k) != target && map.mapInfo[i][j + k] == 1 &&
                        map.searched[i][j + k] == 0) {
                        map.mapInfo[i][j + k] = 0;
                        if (map.calcShortestPath(curr)[target.first][target.second] != -1) {
...
void solve(int seed = -1, bool test = false) {
    ...
    if (test) {
        cerr << "Seed: " << seed << " Score: " << turn << " Size: " << in.N << endl;
    }
    sum += turn;
}
...
int main() {
    ...
    cerr << "Average Score: " << sum / 100 << endl;
    return 0;
}