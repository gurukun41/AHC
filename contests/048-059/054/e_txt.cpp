...
vector<pair<ll, ll>> findInitialBlockingPositions(Map& map, const vector<pair<ll, ll>>& guidePath = {}) {
    ...
    auto tryPlaceTree = [&](ll row, ll col) -> bool {
        // 誘導経路上でないかチェック
        if (find(guidePath.begin(), guidePath.end(), make_pair(row, col)) != guidePath.end()) {
            return false;  // 誘導経路上なので配置しない
        }
...
vector<pair<ll, ll>> findOptimalBlockingPositions(Map& map, bool first) {
    ...
    // 伝説の花がすごく上にあるかどうかを判定
    bool flowerInUpperHalf = map.target.first < map.N / 3;

    // 誘導経路を保存する変数
    vector<pair<ll, ll>> guidePath;

    // 最初のターンなら初期パターンで配置
    if (first) {
        // 伝説の花が上半分にある場合の特別処理
        if (flowerInUpperHalf) {
            // 勇者の初期位置から下の壁への最短経路を構築
            pair<ll, ll> startPos = {0, map.N / 2};  // 勇者の初期位置

            // 花とその周囲8マスを一時的に通行不可にする
            vector<pair<pair<ll, ll>, ll>> originalStates;  // 位置と元の状態を保存
            ...
            for (int dir = 0; dir < 9; dir++) {
                ll nx = map.target.first + dx[dir];
                ll ny = map.target.second + dy[dir];

                if (nx >= 0 && nx < map.N && ny >= 0 && ny < map.N) {
                    // 元の状態を保存
                    originalStates.push_back({{nx, ny}, map.mapInfo[nx][ny]});
                    // 一時的に木として扱う
                    map.mapInfo[nx][ny] = 0;
                }
            }
            ...
            // 下の壁の各位置への距離を計算
            vector<pair<ll, pair<ll, ll>>> bottomWallDistances;
            for (ll j = 0; j < map.N; j++) {
                if (map.mapInfo[map.N - 1][j] == 1) {  // 下の壁の空きマス
                    vvl dist = map.calcShortestPath(startPos);
                    if (dist[map.N - 1][j] != -1) {
                        bottomWallDistances.push_back({dist[map.N - 1][j], {map.N - 1, j}});
                    }
                }
            }

            // 最短距離でソート
            sort(bottomWallDistances.begin(), bottomWallDistances.end());

            if (!bottomWallDistances.empty()) {
                // 最短経路の目標地点
                pair<ll, ll> targetBottom = bottomWallDistances[0].second;

                // 最短経路を逆算で構築
                vvl dist = map.calcShortestPath(startPos);

                // 目標地点から開始位置まで逆算
                pair<ll, ll> current = targetBottom;
                guidePath.push_back(current);  // 誘導経路に追加

                int pathDx[4] = {-1, 1, 0, 0};
                int pathDy[4] = {0, 0, -1, 1};

                while (current != startPos) {
                    bool found = false;
                    for (int dir = 0; dir < 4; dir++) {
                        ll nx = current.first + pathDx[dir];
                        ll ny = current.second + pathDy[dir];

                        if (nx >= 0 && nx < map.N && ny >= 0 && ny < map.N && dist[nx][ny] != -1 &&
                ...
                // 経路反転
                reverse(guidePath.begin(), guidePath.end());
                // 花とその周囲の状態を元に戻す
                for (const auto& state : originalStates) {
                    map.mapInfo[state.first.first][state.first.second] = state.second;
                }

                // 経路の両脇に木を配置
                for (const auto& pathCell : guidePath) {
                    ll pi = pathCell.first, pj = pathCell.second;
                    ...
                    for (int side = -1; side <= 1; side += 2) {  // -1: 左, 1: 右
                        ...
                        if (ni >= 0 && ni < map.N && nj >= 0 && nj < map.N && map.mapInfo[ni][nj] == 1 &&  // 空きマス
                            ...
                            pair<ll, ll>(ni, nj) != map.target &&  // 伝説の花ではない
                            find(guidePath.begin(), guidePath.end(), make_pair(ni, nj)) ==
                                guidePath.end()) {  // 経路上ではない

                            // 一時的に木を配置
                            map.mapInfo[ni][nj] = 0;

                            // 経路が確保されているか確認
                            vvl newDist = map.calcShortestPath(startPos);
                            if (newDist[map.target.first][map.target.second] != -1 && ni != map.N - 1) {
                    ...
                    // 上下に木を配置（経路を狭める）
                    for (int updown = -1; updown <= 1; updown += 2) {  // -1: 上, 1: 下
                        ll ni = pi + updown, nj = pj;
                        ...
                        if (ni >= 0 && ni < map.N && nj >= 0 && nj < map.N && map.mapInfo[ni][nj] == 1 &&  // 空きマス
                            ...
                            // 経路が確保されているか確認
                            vvl newDist = map.calcShortestPath(startPos);
                            if (newDist[targetBottom.first][targetBottom.second] != -1 &&
                                newDist[map.target.first][map.target.second] != -1 && ni != map.N - 1) {
                                result.push_back({ni, nj});
                            } else {
                                map.mapInfo[ni][nj] = 1;
                            }
        ...
        // 初期の渦巻き配置も実行（誘導経路を渡す）
        vector<pair<ll, ll>> spiralResult = findInitialBlockingPositions(map, guidePath);
        result.insert(result.end(), spiralResult.begin(), spiralResult.end());
    ...
    for (const auto& pos : newlySearched) {
        ...
        for (int dir = 0; dir < 2; dir++) {
            ...
            // 花の位置に応じた配置条件
            bool positionCondition;
            if (flowerInUpperHalf) {
                // 花が上半分にある場合：冒険者より上にある位置のみ
                positionCondition = (ni < map.curr.first);
            } else {
                // 花が下半分にある場合：冒険者より下にある位置のみ
                positionCondition = (ni > map.curr.first);
            }
