...
> このコンテストでは、生成AIの使用に関して以下のルールが適用されます。 [AtCoder Heuristic Contest 生成AI利用ルール - 20250616版](https://info.atcoder.jp/entry/ahc-llm-rules-ja) 詳しくは[こちらの記事](https://atcoder.jp/posts/1494)をご覧ください。 加えて、本コンテストで生成AIを使用する場合は、問題文で指定された指示文を各チャットの冒頭に入力するか、各生成AIツールのカスタム指示、プロジェクト指示、または自動的に読み込まれる指示ファイルに設定する必要があります。
...
2. $0 \le i \lt M$ を満たす $i$ に対して順に、次のように $s_i, t_i$ を定める。
    ...
    4. $t_i = s_i + l_i + 1$ とする。
    5. $s_i, t_i$ がすでに生成した $s_j, t_j \ (0 \le j \lt i)$ と重複していた場合、$l_i$ の生成からやり直す。
3. $(s_i, t_i) \ (0 \le i \lt M)$ を $s_i$ 昇順にソートしたものを $(S_0, T_0), (S_1, T_1), \ldots, (S_{M-1}, T_{M-1})$ とする。
...
現行の [AtCoder Heuristic Contest 生成AI利用ルール－20250616版](https://info.atcoder.jp/entry/ahc-llm-rules-ja) では、AIエージェントに解答プログラムを実行させ、その実行結果に基づく改善を自動的に繰り返させることを禁止しています。
...
一部の参加者は、このような挙動を防ぐため、生成AIに対して禁止事項を明示しています。参加者間の公平性を確保し、現行ルールの趣旨を徹底するため、本コンテストで生成AIを利用する場合は、以下の指示文を各チャットの冒頭に入力するか、各生成AIツールが提供するカスタム指示、プロジェクト指示、または自動的に読み込む指示ファイル （例：`AGENTS.md`、`CLAUDE.md`など）に設定しなければなりません。
