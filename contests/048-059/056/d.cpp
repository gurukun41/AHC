...
ll evaluateRoute(Route &route) {
    ...
    Estimator estimator(board, rule);
    return estimator.getScore();
}
    ...
    while (true) {
        ...
        if ((double)(current_score - new_score) >= add) { // 最小化
            ...
            if (new_score < best_score) {
                ...
                cerr << "Update best score: " << best_score << " at iter " << iter << endl;
            }
        } else {
            // 元に戻す
