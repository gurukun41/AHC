...
// ---------- Optuna v29のcompile-time調整値 ----------
// main.cppは変更せず、この別solverだけを-Dで切り替える。全defaultは現行baselineと同じ。
#ifndef AHC069_TUNE_DLP_SCALE_MILLI
#define AHC069_TUNE_DLP_SCALE_MILLI 1000
#endif
#ifndef AHC069_TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI
#define AHC069_TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI 250
#endif
#ifndef AHC069_TUNE_PLACEMENT_GLOBAL_SHORTLIST
#define AHC069_TUNE_PLACEMENT_GLOBAL_SHORTLIST 3
#endif
#ifndef AHC069_TUNE_PLACEMENT_SHORTLIST_LIMIT
#define AHC069_TUNE_PLACEMENT_SHORTLIST_LIMIT 6
#endif
#ifndef AHC069_TUNE_CONNECTED_GROWTH_SEED_LIMIT
#define AHC069_TUNE_CONNECTED_GROWTH_SEED_LIMIT 16
#endif
#ifndef AHC069_TUNE_GROW_AND_TRIM_EXTRA_CELLS
#define AHC069_TUNE_GROW_AND_TRIM_EXTRA_CELLS 8
#endif
#ifndef AHC069_TUNE_GROW_AND_TRIM_CANDIDATE_LIMIT
#define AHC069_TUNE_GROW_AND_TRIM_CANDIDATE_LIMIT 8
#endif
#ifndef AHC069_TUNE_ROOT_FUTURE_WEIGHT_MILLI
#define AHC069_TUNE_ROOT_FUTURE_WEIGHT_MILLI 1000
#endif

constexpr int TUNE_WEIGHT_SCALE = 1000;
constexpr int TUNE_DLP_SCALE_MILLI = AHC069_TUNE_DLP_SCALE_MILLI;
constexpr int TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI = AHC069_TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI;
constexpr int TUNE_ROOT_FUTURE_WEIGHT_MILLI = AHC069_TUNE_ROOT_FUTURE_WEIGHT_MILLI;
static_assert(750 <= TUNE_DLP_SCALE_MILLI && TUNE_DLP_SCALE_MILLI <= 1300);
static_assert(0 <= TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI &&
              TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI <= 500);
static_assert(0 <= TUNE_ROOT_FUTURE_WEIGHT_MILLI && TUNE_ROOT_FUTURE_WEIGHT_MILLI <= 1500);

// ---------- 通常配置 ----------
constexpr int COMPACT_PERIMETER_MARGIN = 4;
constexpr int PLACEMENT_GLOBAL_SHORTLIST = AHC069_TUNE_PLACEMENT_GLOBAL_SHORTLIST;
constexpr int PLACEMENT_SHORTLIST_LIMIT = AHC069_TUNE_PLACEMENT_SHORTLIST_LIMIT;
constexpr int CONNECTED_GROWTH_SEED_LIMIT = AHC069_TUNE_CONNECTED_GROWTH_SEED_LIMIT;
constexpr int GROW_AND_TRIM_EXTRA_CELLS = AHC069_TUNE_GROW_AND_TRIM_EXTRA_CELLS;
constexpr int GROW_AND_TRIM_CANDIDATE_LIMIT = AHC069_TUNE_GROW_AND_TRIM_CANDIDATE_LIMIT;
static_assert(2 <= PLACEMENT_GLOBAL_SHORTLIST && PLACEMENT_GLOBAL_SHORTLIST <= 5);
static_assert(4 <= PLACEMENT_SHORTLIST_LIMIT && PLACEMENT_SHORTLIST_LIMIT <= 8);
static_assert(PLACEMENT_GLOBAL_SHORTLIST <= PLACEMENT_SHORTLIST_LIMIT);
static_assert(8 <= CONNECTED_GROWTH_SEED_LIMIT && CONNECTED_GROWTH_SEED_LIMIT <= 24);
static_assert(4 <= GROW_AND_TRIM_EXTRA_CELLS && GROW_AND_TRIM_EXTRA_CELLS <= 12);
static_assert(4 <= GROW_AND_TRIM_CANDIDATE_LIMIT && GROW_AND_TRIM_CANDIDATE_LIMIT <= 12);
...
// root比較では現在差と未来差を1000倍整数で合成し、重み変更後も決定性を保つ。
i128 tuned_root_margin_scaled(int scenario_count, ll direct_gain, i128 future_delta_sum) {
    return (i128)TUNE_WEIGHT_SCALE * scenario_count * direct_gain +
           (i128)TUNE_ROOT_FUTURE_WEIGHT_MILLI * future_delta_sum;
}

ll tuned_root_margin_scaled_ll(int scenario_count, ll direct_gain, ll future_delta_sum) {
    i128 value = tuned_root_margin_scaled(scenario_count, direct_gain, future_delta_sum);
    assert(numeric_limits<ll>::min() <= value && value <= numeric_limits<ll>::max());
    return (ll)value;
}

bool tuned_root_scenario_positive(ll direct_gain, ll future_delta) {
    return (i128)TUNE_WEIGHT_SCALE * direct_gain +
               (i128)TUNE_ROOT_FUTURE_WEIGHT_MILLI * future_delta >
           0;
}

long double tuned_root_margin_fee(i128 scaled_margin, int scenario_count) {
    return (long double)scaled_margin / (TUNE_WEIGHT_SCALE * scenario_count);
}
...
long double evaluate_compact_fit(const vs &park, const vvi &owner, const vector<GroupState> &groups,
    ...
    // 平均と最悪snapshotを調整可能な比率で混ぜ、途中で空き形状が崩れる候補を避ける。
    long double average = sum / FUTURE_FIT_SNAPSHOT_COUNT;
    long double minimum_weight =
        (long double)TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI / TUNE_WEIGHT_SCALE;
...
ArrivalDecision evaluate_arrival_decision(const vs &park, const vvi &decision_owner,
    ...
    // DLPの形は保ったまま価格水準だけを較正し、実ターンと仮想未来へ同じ倍率を使う。
    opportunity_cost *= (long double)TUNE_DLP_SCALE_MILLI / TUNE_WEIGHT_SCALE;
    if (root_alternatives) root_alternatives->clear();
...
bool confirm_root_override(
    ...
    for (int scenario_index = 0; scenario_index < ROOT_CONFIRM_SCENARIO_COUNT; scenario_index++) {
        ...
        positive_scenario[scenario_index] = tuned_root_scenario_positive(direct_delta, future_delta);
    ...
    i128 margin_times_scenarios =
        tuned_root_margin_scaled(ROOT_CONFIRM_SCENARIO_COUNT, direct_delta, future_delta_sum);
    long double holdout_margin =
        tuned_root_margin_fee(margin_times_scenarios, ROOT_CONFIRM_SCENARIO_COUNT);
...
optional<RootActionResult> choose_root_action_with_rescue(
    ...
    // Push-outの経済gateにも通常admissionと同じDLP倍率を適用する。
    opportunity_cost *= (long double)TUNE_DLP_SCALE_MILLI / TUNE_WEIGHT_SCALE;
    const GroupState &arrival = groups[arrival_id];
    int minimum_perimeter = compact_shapes[arrival.p].front().perimeter;
    ...
    if (rollout_ready) {
        ...
        for (int candidate_index = 0; candidate_index < (int)candidates.size(); candidate_index++) {
            ...
            evaluation.margin_twice = tuned_root_margin_scaled_ll(
            ...
            diagnostics.rollout_slot_margin[candidate_index] +=
                tuned_root_margin_fee(evaluation.margin_twice, RESCUE_ROLLOUT_SCENARIO_COUNT);
            bool first_accepts =
                tuned_root_scenario_positive(candidate.direct_gain, evaluation.future_delta[0]);
            bool second_accepts =
                tuned_root_scenario_positive(candidate.direct_gain, evaluation.future_delta[1]);
        ...
        long double rollout_margin =
            tuned_root_margin_fee(best_evaluation.margin_twice, RESCUE_ROLLOUT_SCENARIO_COUNT);
        bool first_accepts =
            tuned_root_scenario_positive(best_candidate.direct_gain, best_evaluation.future_delta[0]);
        bool second_accepts =
            tuned_root_scenario_positive(best_candidate.direct_gain, best_evaluation.future_delta[1]);
        ...
        if (candidates.size() == 2) {
            ...
            diagnostics.rollout_width_predicted_gain += tuned_root_margin_fee(
        ...
        if constexpr (!ROOT_PROTECTED_ONLY) {
            for (int source_index : available_alternatives) {
                ...
                alternative_evaluation.margin_twice = tuned_root_margin_scaled_ll(
                ...
                diagnostics.root_alternative_margin += tuned_root_margin_fee(
                    alternative_evaluation.margin_twice, RESCUE_ROLLOUT_SCENARIO_COUNT);
                bool alternative_first_accepts =
                    tuned_root_scenario_positive(alternative_direct_gain,
                                                 alternative_evaluation.future_delta[0]);
            ...
            long double screen_gain = tuned_root_margin_fee(
...
optional<RootActionResult> choose_normal_root_action(
    ...
    long double screen_gain = tuned_root_margin_fee(best_margin_twice, ROOT_SCREEN_SCENARIO_COUNT);
    diagnostics.root_confirmation_screen_gain += screen_gain;
...
int main() {
    ...
    cerr << "accepted=" << accepted_count << " rejected=" << rejected_count
         << " tune_version=29"
         << " tune_dlp_scale_milli=" << TUNE_DLP_SCALE_MILLI
         << " tune_future_fit_min_weight_milli=" << TUNE_FUTURE_FIT_MIN_WEIGHT_MILLI
         << " tune_placement_global_shortlist=" << PLACEMENT_GLOBAL_SHORTLIST
         << " tune_placement_shortlist_limit=" << PLACEMENT_SHORTLIST_LIMIT
         << " tune_connected_growth_seed_limit=" << CONNECTED_GROWTH_SEED_LIMIT
         << " tune_grow_and_trim_extra_cells=" << GROW_AND_TRIM_EXTRA_CELLS
         << " tune_grow_and_trim_candidate_limit=" << GROW_AND_TRIM_CANDIDATE_LIMIT
         << " tune_root_future_weight_milli=" << TUNE_ROOT_FUTURE_WEIGHT_MILLI
