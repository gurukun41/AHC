...
struct Route {
    ...
    Route() {}
        ...
        rep(i,0,K-1) {
            ...
            // A*アルゴリズム（タイブレーキング付き）
            // f(n) = g(n) + h(n) + tie_breaker(n)
            // tie_breaker: 同じf値の場合の優先度（中央に近い方を優先）
            auto heuristic = [&](ll pi, ll pj) -> ll {
                return abs(pi - gi) + abs(pj - gj);
            };
            
            // タイブレーカー: 中央への近さ（微小な値で加算）
            auto tie_breaker = [&](ll pi, ll pj) -> ld {
                ld ci = N / 2.0;
                ld cj = N / 2.0;
                // 0.001などの微小な値を掛けることで、最短性を保ちつつ優先度を付ける
                return (abs(pi - ci) + abs(pj - cj)) * 0.001;
            };
            ...
            map<pl, ll> gScore; // スタートからの実コスト
            
            pl start = {si, sj};
            ld f_start = heuristic(si, sj) + tie_breaker(si, sj);
