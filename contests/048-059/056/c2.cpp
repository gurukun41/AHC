...
struct Route {
    ...
    void DecideRoute() {
        rep(i,0,K-1) {
            ...
            // f(n) = g(n) + h(n)
            // g(n): スタートからnまでの実コスト
            // h(n): nからゴールまでの推定コスト（マンハッタン距離）
            auto heuristic = [&](ll pi, ll pj) -> ll {
                return abs(pi - gi) + abs(pj - gj);
            };

            // priority_queue: (f値, (g値, 座標))
            priority_queue<pair<ll, pair<ll, pl>>, vector<pair<ll, pair<ll, pl>>>, greater<>> pq;
            map<pl, pl> parent; // 経路復元用
            map<pl, ll> gScore; // スタートからの実コスト
            
            pl start = {si, sj};
            pq.push({heuristic(si, sj), {0, start}});
            parent[start] = {-1, -1};
            gScore[start] = 0;
            bool found = false;
                ...
                for (const auto& dir : directions) {
                    ...
                    if (ni >= 0 && ni < N && nj >= 0 && nj < N) {
                        ...
                        if (!gScore.count(next) || tentative_g < gScore[next]) {
                            ...
                            ll f = tentative_g + heuristic(ni, nj);
                            parent[next] = curr;
