...
struct Route {
    ...
    vector<bool> boundaryLocked; // 隣接する経路同士を結合しない境界のフラグ

    Route() {}

    void refreshUsedCells() {
        usedCells.clear();
        for (const auto& path : paths) {
            ...
            boundaryLocked.assign(max(0LL, (ll)paths.size() - 1), false);
            refreshUsedCells();
    }
    ...
    // 連続する経路を可能な限り連結（ロックされた境界は維持）
    void mergePaths() {
        if (paths.empty()) {
            boundaryLocked.clear();
            return;
        }

        ll before = paths.size();
        boundaryLocked.resize(max(0LL, (ll)paths.size() - 1), false);

        vector<vpl> merged;
        vector<bool> newBoundary;
        vpl current = paths[0];

        for (ll i = 0; i < (ll)paths.size() - 1; i++) {
            if (!boundaryLocked[i] && canMergePaths(current, paths[i + 1])) {
                current.insert(current.end(), paths[i + 1].begin() + 1, paths[i + 1].end());
            } else {
                merged.push_back(current);
                newBoundary.push_back(boundaryLocked[i]);
                current = paths[i + 1];
            }
        }
    ...
    void optimizeGlobalRoute(double tl) {
        ...
        auto calcMetrics = [&]() -> pair<ll, ll> {
            ll totalDist = 0;
            for (const auto& path : paths) {
                totalDist += max(0LL, (ll)path.size() - 1);
            }
            return {totalDist, (ll)usedCells.size()};
        };

        auto bfs_path = [&](const pl& startCell, const pl& goalCell) -> vpl {
            ll NN = N * N;
            vector<int> parent(NN, -1);
            queue<ll> q;
            ll s_id = cell_id(startCell.first, startCell.second);
            ll g_id = cell_id(goalCell.first, goalCell.second);
            q.push(s_id);
            parent[s_id] = s_id;
        ...
        vector<bool> best_boundary = boundaryLocked;
        ...
        while (get_time() < deadline) {
            ...
            bool isSplitMove = false;
            ll seg_idx = -1;
            vpl old_path1, old_path2;
            struct SplitContext {
                ll idx = -1;
                vpl original;
            } splitCtx;

            double choice = rnd.next_double();
            if (choice < 0.2) {
                vector<ll> candidates;
                rep(i, 0, (ll)paths.size()) {
                    if ((ll)paths[i].size() >= 3) candidates.push_back(i);
                }
                if (candidates.empty()) continue;
                splitCtx.idx = candidates[rnd.next_int(candidates.size())];
                auto &targetPath = paths[splitCtx.idx];
                ll splitPos = 1 + rnd.next_int((ll)targetPath.size() - 2);
                splitCtx.original = targetPath;
                vpl first(targetPath.begin(), targetPath.begin() + splitPos + 1);
                vpl second(targetPath.begin() + splitPos, targetPath.end());
                paths[splitCtx.idx] = first;
                paths.insert(paths.begin() + splitCtx.idx + 1, second);
                boundaryLocked.insert(boundaryLocked.begin() + splitCtx.idx, true);
                isSplitMove = true;
            } else {
                if (paths.size() < 2) continue;
                seg_idx = rnd.next_int((ll)paths.size() - 1);
                auto &seg1 = paths[seg_idx];
                auto &seg2 = paths[seg_idx + 1];
                if (seg1.empty() || seg2.empty()) continue;
                pl s = seg1.front();
                pl middle = seg1.back();
                if (seg2.front() != middle) continue;
                pl e = seg2.back();

                vpl path_sm = bfs_path(s, middle);
                vpl path_me = bfs_path(middle, e);
            ...
            if (total_moves > T) {
                if (isSplitMove) {
                    paths.erase(paths.begin() + splitCtx.idx + 1);
                    paths[splitCtx.idx] = splitCtx.original;
                    boundaryLocked.erase(boundaryLocked.begin() + splitCtx.idx);
                } else {
            ...
            if (new_score < best_score) {
                ...
                if (rnd.next_double() < prob) accept = true;
            }
