...
void MakeRuleAndBoardByPath(Rule &rule, Board &board, Route &route) {
    ...
    set<pl> goalCells;
    rep(p, 0, K-1) {
        goalCells.insert(targets[p]);
    }

    // 交差点の方向パターンを収集（ただしゴールマスは除外）
    map<pl, vector<char>> crossDirPatterns;
    for (auto &c : crossCells) {
        // ゴールマスは交差点パターンに含めない
        if (goalCells.count(c)) continue;
    ...
    // すべての経路の開始マス（ゴール）を固有の特別色に
    rep(p, 0, K-1) {
        pl start = targets[p];
        // ゴールマスは常に固有色（交差点パターンとは独立）
        cellPatternColors[start] = {nextColor++};
    }
    ...
    rep(p, 0, K-1) {
        ...
        if (route.paths[p].size() <= 1) {
            pl s = targets[p];
            if (board.colors[s.first][s.second] == 0 && cellPatternColors.count(s)) {
                board.paint(s.first, s.second, cellPatternColors[s][0]);
            }
        }
    }
    ...
    rep(k, 0, useSteps) {
        ...
        // ゴールマスの場合は固有色を維持（変更しない）
        if (goalCells.count(st.c)) {
            newColor = cellPatternColors[st.c][0]; // ゴールの固有色
        } else if (cellPatternColors.count(st.c)) {
            // 交差点の場合、次の段階の色に進める
            ...
            if (stage + 1 < (ll)cellPatternColors[st.c].size()) {
                ...
                // 最後の段階の場合、そのままの色を維持
                newColor = cellPatternColors[st.c].back();
            }
        }
