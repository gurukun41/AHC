...
注意
このコンテストでは、生成AIの使用に関して以下のルールが適用されます。
AtCoder Heuristic Contest 生成AI利用ルール - 20250616版
詳しくはこちらの記事をご覧ください。
...
出発線および待避線には、それぞれ左から右へ順に $0,1,\dots,R-1$ の番号が付けられている。
...
各車両には $0,\dots,10R-1$ の ID が 1 つずつ振られており、出発線 $r$ の先頭から $c$ 両目に配置されている車両の ID は $Y_{r,c}$ である。待避線はすべて空である。
出発線と待避線の間で車両を移動させることによって、各出発線 $r$ について、ID $10r, 10r+1, \dots, 10r+9$ の車両を先頭から順に並べることが目標である。

各ターンでは、以下の 2 種類の移動を複数同時に行うことができる。各移動は連続した $k$ 両（$k \ge 1$）を対象とし、その順序は保存される。
...
$Y_{r,c}$ は、初期状態において出発線 $r$ の先頭から $c$ 両目（$0 \le c < 10$）に配置されている車両の ID である。

$Y_{r,c}$ は $0,\dots,10R-1$ をちょうど 1 回ずつ含む。
...
ここで、$T$ は操作を行うターン数（$T \le 4000$）である。
$T$ を出力した後、各ターン $t = 0, 1, \dots, T-1$ について、以下をこの順に出力する。

まず、そのターンで実行する移動数 $K_t$ を 1 行で出力する（$1 \le K_t \le R$）。

続く $K_t$ 行に、そのターンで実行する各移動を type i j k の形式（$\mathrm{type} \in \{0,1\}$, $0 \le i,j < R$, $k \ge 1$）で出力する。
...
整数列 $0,1,\dots,10R-1$ を一様ランダムに並び替え、並び替えた列を順に $Y_{0,0}, Y_{0,1}, \dots, Y_{0,9}, Y_{1,0}, \dots, Y_{R-1,9}$ とする。
