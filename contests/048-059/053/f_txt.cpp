...
struct UsableNumber {
    ...
    UsableNumber(Input& in) {
        ...
        rep(i,0,in.N/8) {
            numbers[i*4] = sepsep * (i+1);
            numbers[i*4+1] = sepsep * (i+1);
            numbers[i*4+2] = sepsep * (i+1);
            numbers[i*4+3] = sepsep * (i+1);
            if(i == in.N/8 -1) {
                numbers[i*4+4] = sepsep * (i+2);
                numbers[i*4+5] = sepsep * (i+2);
            }
        }
        rep(i,in.N/2,in.N) {
...
int main(){
    ...
    string output_filename = "out/f_" + oss.str() + ".txt";
