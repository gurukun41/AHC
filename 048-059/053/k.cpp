...
struct UsableNumber {
    ...
    UsableNumber(Input& in) {
        ...
        rep(i, 0, in.N / 5) {
            ...
            numbers[i * 3] = sepsep * (i + 1 - in.N / 60);
            numbers[i * 3 + 1] = sepsep * (i + 1 - in.N / 60);
            numbers[i * 3 + 2] = sepsep * (i + 1 - in.N / 60);
        }
...
void greedy(Mountain & mt, UsableNumber & un, const Input & in) {
    ...
    rep(i, in.N/20*11, in.N/5*3) {
        if (!un.used[i]) {
            ...
            rep(j, 0, in.M) {
                ll total_diff = 0;
                if(mt.sum[j] + un.numbers[i] - mt.target[j] > 0) continue; // 超過する場合はスキップ
...
void simulated_annealing(Mountain & mt, UsableNumber & un, const Input & in) {
    ...
    while (true) {
        ...
        // 温度スケジュール
        T = T_start * (1.0 - elapsed / TIME_LIMIT) + T_end * (elapsed / TIME_LIMIT);
